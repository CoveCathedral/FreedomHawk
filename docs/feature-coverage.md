...
| Metronome | ✅ | **New (the pedal had none)** — tempo (30–300 BPM, announced as real BPM), time signature incl. **odd/prog meters** with accent grouping (e.g. 2+2+3 for 7/8), subdivision, tap-tempo. Keeps running across tabs |
| Sequin (drum sequencer) | ✅ | **New (the pedal had none)** — synth kit or your own drum libraries; **500-groove library across ~60 genres + your saved patterns** (genre categories, filtering, WAV/pattern-file/MIDI export & import); **tracker-grid Pattern Editor** (spoken cursor, drum stacking, mix-and-match across libraries, save-as-preset, Ctrl+D anywhere). Feel: per-hit **dynamics** (accent/ghost), **swing**, **humanize**, **polymeter** (per-line loop lengths). Per line: **tuning** (`[`/`]`, spoken notes via by-ear **pitch detection**), **volume** (`,`/`.` dB), **choke groups** (`C`). Practice: **count-in**, **tempo trainer** (climbs BPM, spoken). See `docs/drum-kits.md` |
...
- A full **accessible drum machine** the pedal never had — 500 grooves + saved patterns
