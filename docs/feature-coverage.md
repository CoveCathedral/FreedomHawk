...
| Drum Looper | ✅ | **New (the pedal had none)** — synth kit or your own drum libraries; **500-groove library across ~60 genres + your saved patterns** (genre categories, filtering, WAV/pattern-file/MIDI export & import); **tracker-grid Pattern Editor** (spoken cursor, drum stacking, mix-and-match across libraries, save-as-preset, Ctrl+D anywhere). Feel: per-hit **dynamics** (accent/ghost), **swing**, **humanize**, **polymeter** (per-line loop lengths). Per line: **tuning** (`[`/`]`, spoken notes via by-ear **pitch detection**), **volume** (`,`/`.` dB), **choke groups** (`C`). Practice: **count-in**, **tempo trainer** (climbs BPM, spoken). See `docs/drum-kits.md` |
